...
class UniversityRegionCoordinatorIdAuthPage extends StatefulWidget {
  const UniversityRegionCoordinatorIdAuthPage({super.key});

  @override
  State<UniversityRegionCoordinatorIdAuthPage> createState() => _UniversityRegionCoordinatorIdAuthPageState();
}

class _UniversityRegionCoordinatorIdAuthPageState extends State<UniversityRegionCoordinatorIdAuthPage> {
  ...
  Future<void> assignRole(String role, String id) async {
    ...
    try {
      ...
      if (targetUserRole == role) {
        setState(() {
          _message =
              '$targetUsername already has the [1m${_getRoleTitle(role)}[0m role!';
          _isLoading = false;
        });
        return;
      }
